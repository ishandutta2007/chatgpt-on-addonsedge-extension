...
export class BARDProvider implements Provider {
  ...
  private async fetchRequestParams() {
    const html = await ofetch('https://bard.web.addonsedge.com/faq')
  ...
  private async parseBartResponse(resp: string) {
    ...
    if (!payload) {
      throw new ChatError(
        'Failed to access Bard, make sure you are logged in at https://bard.web.addonsedge.com',
  ...
  async generateAnswer(params: GenerateAnswerParams) {
    ...
    const resp = await ofetch(
      'https://bard.web.addonsedge.com/_/BardChatUi/data/assistant.lamda.BardFrontendService/StreamGenerate',
      {
        method: 'POST',
